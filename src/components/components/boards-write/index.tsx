            ...
            <div className={styles.imgUpload}>
              {images?.map((image, index) => (
                <div key={index} className={styles.imageUploadButton}>
                  ...
                  />
                  <div>
                    {/* 파일 입력 필드와 연결된 레이블로 클릭 시 파일 선택 창이 열림 */}
                    <label htmlFor={`file-input-${index}`} style={{ cursor: "pointer" }}>
                      {/* 이미지 미리보기: 이미지가 있으면 해당 URL을, 없으면 기본 이미지를 표시 */}
                      {!props.isEdit ? (
                        <Image
                          unoptimized={true}
                        ...
                        <Image
                          ...
                          src={
                            image === ""
                              ? defaultImage
                              : `https://storage.googleapis.com/${editImages?.[index]}` || image
                          }
                          alt={`Preview ${index + 1}`}
                          className={styles.imagePreview}
                          width={150}
